...
if(!exists('ac.new')){
	...
	noi.irl.af<-readRDS('data/noi_irl_region_af_mean.rds')
    #noi.mean.ancestry<-colMeans(noi.cl)
    #irl.mean.ancestry<-colMeans(irl.cl)
	noi.mean.ancestry<-colMeans(noi.irl.af[[1]])
	irl.mean.ancestry<-colMeans(noi.irl.af[[2]])

    ac.entropy.gb<-apply(as.matrix(ac.new[,1:23]),1,make.entropy.sub)

    #spdf<-readRDS('v2_self_BI_487409.rds')
    #spdf@data<-ac.new
	spdf<-readRDS('data/gwas_snp_regional_plot_af.rds')
}

if(!exists('r.af')){
	r.af<-readRDS('data/regions_af.rds')
}
...
plot.maf.gbirl<-function(n,q,dir){
    if(!dir.exists(dir)){
        dir.create(dir)
    }
    ...
    for(i in 1:nrow(px.mean.gb)){
		rsid<-colnames(spdf@data)[i]
        pdf(paste0(dir,'/GB_regions_',colnames(spdf@data)[i],'_',n,'_',q,'.pdf'))
        par(bty='n')
        plot.px[plot.px.gbidx]<-px.mean.gb[i,]
        plot.px[plot.px.noiidx]<-r.af[i,'G_Noi']
        plot.px[plot.px.irlidx]<-r.af[i,'G_Ireland']
